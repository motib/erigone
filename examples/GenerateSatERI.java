/*
  ...
  Version for the Erigone model checker.
...
public class GenerateSatERI {
  ...
  static void writeBatch(String name, int r) {
    if (r == 0) {
      batchWriter.println("erigone -dr -s -lt10000 " + name);
    } else {
      for (int i = 0; i < runs; i++)
        batchWriter.println("erigone -dr -s -lt10000" +
          " -n" + (int) (Math.random()*100) + " " + name);
    }              
  }
